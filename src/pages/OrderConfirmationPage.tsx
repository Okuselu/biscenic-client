    ...
    const fetchOrder = async () => {
      try {
        const response = await axios.get(
          API_ENDPOINTS.orders.byId(orderId!),
          {
            headers: {
  ...
  const calculateSubtotal = (): number => {
    ...
    if (order.orderItems && order.orderItems.length > 0) {
      return order.orderItems.reduce(
        (sum: number, orderItem: { price: any; quantity: any }) => {
          const price =
  ...
  return (
    <div className="container py-3 py-md-5">
      ...
      <div className="text-center mb-4 mb-md-5">
        ...
        <div
          className="d-flex flex-column flex-sm-row align-items-center justify-content-center gap-2 gap-sm-3 p-3 border rounded shadow-sm"
          style={{
        ...
        >
          ...
          <div
            className="vr opacity-50 d-none d-sm-block"
            style={{ borderColor: "var(--border-color)" }}
          ></div>
          <hr
            className="w-50 d-sm-none my-2"
            style={{ borderColor: "var(--border-color)" }}
          />
          <div className="text-center text-sm-start">
            <small className="d-block" style={{ color: "var(--text-muted)" }}>
