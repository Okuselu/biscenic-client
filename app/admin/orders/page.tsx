...
// API Hooks
import { useOrders, useUpdateOrderStatus, useDeleteOrder, useOrderStats } from "@/services/orders/queries"
...
function OrderBadge({ status }: { status: string }) {
  ...
  return (
    <Badge className={`${statusMap[status.toLowerCase()] || "bg-gray-100 text-gray-800 border-gray-200"} border`}>
...
// Stats card
function OrderStatsCard({ label, value, color }: { label: string; value: string | number; color: "blue" | "green" | "yellow" | "purple" }) {
  const colorMap = {
...
// Item images stack
function OrderItemImageStack({ items }: { items: OrderResponse["orderItems"] }) {
  const getMainImage = (images: any[]) => {
    if (!images?.length) return "/placeholder.svg?height=50&width=50"
      ...
      <div className="flex -space-x-2">
        {items?.slice(0, 3).map((item, idx) => (
          item?.product ? (
            ...
            <div key={idx} className="w-8 h-8 rounded-full border-2 border-white bg-gray-200 flex items-center justify-center">
      ...
      <div className="flex min-h-screen items-center justify-center bg-gray-100">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>Failed to load orders. Please check your connection and try again.</CardDescription>
          </CardHeader>
          <CardContent>
  ...
  return (
    <div className="flex min-h-screen flex-col items-center bg-gray-100 px-4 py-12">
      <Card className="w-full max-w-7xl shadow-lg">
        {/* Header */}
        <CardHeader className="flex flex-col lg:flex-row items-start lg:items-center justify-between pb-4 space-y-4 lg:space-y-0">
          <div>
          ...
          {/* Stats */}
          {stats && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-center w-full lg:w-auto">
              <OrderStatsCard label="Total Orders" value={stats.totalOrders} color="blue" />
              <OrderStatsCard label="Pending" value={stats.pendingOrders} color="yellow" />
              <OrderStatsCard label="Completed" value={stats.completedOrders} color="green" />
              <OrderStatsCard label="Revenue" value={formatCurrency(stats.totalRevenue)} color="purple" />
            </div>
          )}
        </CardHeader>
              ...
              <TableBody>
                {orders.map(order => (
                  <TableRow key={order._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <TableCell>
                      <OrderItemImageStack items={order.orderItems} />
                    </TableCell>
                    ...
                    <TableCell className="flex justify-center space-x-2">
                      <Button variant="ghost" size="icon" onClick={() => handleViewDetails(order)} className="hover:bg-blue-100">
                        <Eye className="h-4 w-4 text-blue-600" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleUpdateStatus(order)} className="hover:bg-green-100" disabled={updateStatusMutation.isPending}>
      ...
      <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        <DialogPortal>
          ...
          <DialogContent className="fixed left-[50%] top-[50%] grid w-full max-w-4xl max-h-[90vh] translate-x-[-50%] translate-y-[-50%] gap-4 border bg-white p-6 shadow-2xl overflow-y-auto rounded-lg" style={{ zIndex: 9999 }}>
            <DialogHeader>
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    {selectedOrder.orderItems?.map((item, idx) => (
                      <div key={idx} className="flex items-center space-x-4 p-3 border rounded-lg">
                        {item?.product ? (
                          <>
                            <Image src={item.product.images?.[0]?.url || "/placeholder.svg"} alt={item.product.name || "Product"} width={60} height={60} className="rounded-md object-cover" />
                ...
                <div className="border-t pt-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xl font-bold text-green-600">{formatCurrency(selectedOrder.totalAmount)}</span>
                  </div>
                  {selectedOrder.shipment && <p className="text-sm text-gray-600 mt-2">Shipment ID: {selectedOrder.shipment}</p>}
                </div>
              </div>
            )}
      ...
      <Dialog open={isStatusModalOpen} onOpenChange={setIsStatusModalOpen}>
        <DialogContent className="max-w-md bg-white">
          <DialogHeader>
            ...
            <DialogDescription>Change the status of order #{selectedOrder?._id.slice(-6).toUpperCase()}</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsStatusModalOpen(false)} disabled={updateStatusMutation.isPending}>Cancel</Button>
            <Button onClick={confirmStatusUpdate} disabled={updateStatusMutation.isPending || !newStatus || newStatus === selectedOrder?.status} className="bg-green-600 text-white hover:bg-green-700">
