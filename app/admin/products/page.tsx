...
export default function AdminProductsPage() {
  ...
  const [products, setProducts] = useState<Product[]>([
    {
      ...
      price: 49.99,
      stock: 12,
      category: { _id: "cat1", name: "Furniture" },
      images: [{ url: "/images/chair.jpg", publicId: "chair1", isMain: true }]
    },
    {
      _id: "2",
      name: "Sample Table",
      ...
      stock: 5,
      category: { _id: "cat1", name: "Furniture" },
      images: [{ url: "/images/table.jpg", publicId: "table1", isMain: true }]
    }
  ])
  const categories = [
    { _id: "cat1", name: "Furniture" },
  ...
  // ===== Helpers =====
  const resetForm = () => {
    setFormData({ name:"", description:"", price:"", stock:"", category:"", images:[], existingImages:[], imagesToKeep:[] })
    setImagePreviewUrls([])
  ...
  const removeExistingImage = (publicId: string) => {
    setFormData(prev => ({ ...prev, imagesToKeep: prev.imagesToKeep?.filter(id => id !== publicId) }))
  }
    ...
    if (editingProduct) {
      setProducts(prev => prev.map(p => 
        p._id === editingProduct._id ? { 
          ...
          images: [...(formData.existingImages?.filter(img => formData.imagesToKeep?.includes(img.publicId)) || []),
                   ...formData.images.map((file, idx) => ({ url: URL.createObjectURL(file), publicId: `new-${idx}` })) ]
        } : p
      ))
    } else {
      const newProduct: Product = {
        ...
        stock,
        category: categories.find(c => c._id === formData.category)!,
        images: formData.images.map((file, idx) => ({ url: URL.createObjectURL(file), publicId: `new-${idx}` }))
      }
  ...
  // ...keep your JSX for table and modals mostly the same
  // just remove any references to mutation hooks

  return (
    <div className="flex min-h-screen flex-col items-center bg-gray-100 px-4 py-12 relative">
      {/* Rest of your JSX here, using the local `products` array */}
      <Card className="w-full max-w-7xl shadow-lg">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <div>
            <CardTitle className="text-2xl font-bold tracking-wide">Product Management</CardTitle>
            <CardDescription>Manage your store's product catalog. {products.length} products total.</CardDescription>
          </div>
          <Button onClick={handleAddProduct} className="bg-black text-white hover:bg-gray-800 flex items-center gap-2">
